 ...
 * @returns {Promise<Utilities.Roblox.UserPresence|Utilities.Roblox.UserPresence[]>} - A list of users representing search results; User presence Type ['Offline' = 0, 'Online' = 1, 'InGame' = 2, 'InStudio' = 3, 'Invisible' = 4].
  ...
  return Axios.post(RequestURL, Payload).then((Res) => {
    const Results = IdsArray.map((UserID) => {
      return Res.data.userPresences.find(
        (/** @type {Utilities.Roblox.UserPresence} */ UserObject) => UserObject.userId === UserID
      );
    });
